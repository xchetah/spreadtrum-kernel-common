...
struct sdcardfs_sb_info {
	...
	char *obbpath_s;
	struct path obbpath;
	void *pkgl_id;		/* sharing */
	gid_t gid;
	mode_t mask;
#ifdef CONFIG_SECURITY
	struct cred *cred;		/* mount process cred info */
#endif
};
...
extern void * packagelist_create(gid_t write_gid, bool is_androidm);
extern void packagelist_destroy(void *pkgl_id);
extern int packagelist_init(void);
extern void packagelist_exit(void);

/* for derived_perm.c */
extern void fix_derived_permission_androidm(struct sdcardfs_sb_info* sbi, struct inode* inode);
extern void setup_derived_state(struct inode *inode, perm_t perm,
				userid_t userid, uid_t uid, gid_t gid, mode_t mode);
extern void setup_derived_state_androidm(struct inode *inode, perm_t perm,
