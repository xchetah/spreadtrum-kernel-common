 ...
 *  Authors: neilwang modify it to compatible with androidm
 *
				...
				int *debug, struct sdcardfs_mount_options *opts)
{
	...
	opts->reserved_mb = 0;
	/* by default, is_androidm is false */
	opts->is_androidm = false;
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		case Opt_token:
			if (match_int(&args[0], &option))
				return 0;
			opts->token = option;
			/* only androidm has such mount option */
			opts->is_androidm = true;
			opts->write_gid = -1;
			break;
		case Opt_full_write:
			if (match_int(&args[0], &option))
				return 0;
			opts->full_write = option;
			break;
		case Opt_label:
			opts->label = match_strdup(&args[0]);
			break;
...
invalid_option:
	...
	if (*debug) {
		...
		printk( KERN_INFO "sdcardfs : options - gid:%d\n",
							opts->fs_low_gid);
		printk( KERN_INFO "sdcardfs : options - is_androidm:%d\n",
							opts->is_androidm);
	}
						...
						void *raw_data, int silent)
{
	...
	if (sb_info->options.is_androidm) {
		if(sb_info->options.multi_user) {
			/* Multi-user storage is fully isolated per user , so "others"
			 * permissoons are completely masked off*/
			switch (sb_info->options.token) {
			case 0:
				/* default */
				sb_info->gid = AID_SDCARD_RW;
				sb_info->mask = 0006;
				break;
			case 1:
				/* read */
				sb_info->gid = AID_EVERYBODY;
				sb_info->mask = 0027;
				break;
			case 2:
				/* write */
				sb_info->gid = AID_EVERYBODY;
				sb_info->mask = sb_info->options.full_write ? 00007 : 0027;
				break;
			}
		} else {
			/* Physical storage is readdable by all users on device, but
			 * the Android  directores are masked off the a single user
			...
			case 1:
				sb_info->gid = AID_EVERYBODY;
				sb_info->mask = sb_info->options.full_write ? 0027 : 0022;
				break;
			case 2:
				sb_info->gid = AID_EVERYBODY;
				sb_info->mask = sb_info->options.full_write ? 0007 : 0022;
				break;
			}
		}
	}
	...
	if (sb_info->options.is_androidm) {
		if (0 == sb_info->options.token) {
			pkgl_id = packagelist_create(sb_info->options.write_gid, sb_info->options.is_androidm);
			...
			if (g_pkgl_id) {
				sb_info->pkgl_id = g_pkgl_id;
			}
			if (2 == sb_info->options.token) {
				g_pkgl_id = NULL;
			}
		}
	} else {
	...
	if (!err) {
		...
		if (sb_info->options.is_androidm) {
			...
			if (sb_info->options.multi_user){
				...
				setup_derived_state_androidm(sb->s_root->d_inode,
						    PERM_ROOT, 0, AID_ROOT, AID_SDCARD_R, false);
			...
			fix_derived_permission_androidm(sb_info, sb->s_root->d_inode);
...
#ifdef CONFIG_SECURITY
static inline void fill_cred(struct super_block *s)
{
	struct sdcardfs_sb_info *sbi = SDCARDFS_SB(s);
	sbi->cred = prepare_creds();
}
#endif
