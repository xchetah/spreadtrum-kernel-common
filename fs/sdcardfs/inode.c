...
const struct cred * override_fsids(struct sdcardfs_sb_info* sbi)
{
	...
	security_transfer_creds(cred, sbi->cred);
#endif
...
static void sdcardfs_notify_delete(struct sdcardfs_sb_info *sbi, char *path)
{
	char *validate_path = __getname();

	struct path saved_path;
	if (IS_ERR(path)) {
		__putname(validate_path);
		return;
	} else {
		if (sbi->options.token != 0) {
			memset(validate_path, 0, sizeof(validate_path));
			strcpy(validate_path, "/mnt/runtime/default/");
			strcat(validate_path, sbi->options.label);
			strcat(validate_path, path);
			kern_path(validate_path, LOOKUP_FOLLOW | LOOKUP_DIRECTORY, &saved_path);
		}
		if (sbi->options.token != 1) {
			memset(validate_path, 0, sizeof(validate_path));
			strcpy(validate_path, "/mnt/runtime/read/");
			strcat(validate_path, sbi->options.label);
			strcat(validate_path, path);
			kern_path(validate_path, LOOKUP_FOLLOW | LOOKUP_DIRECTORY, &saved_path);
		}
		if (sbi->options.token != 2) {
			memset(validate_path, 0, sizeof(validate_path));
			strcpy(validate_path, "/mnt/runtime/write/");
			strcat(validate_path, sbi->options.label);
			strcat(validate_path, path);
			kern_path(validate_path, LOOKUP_FOLLOW | LOOKUP_DIRECTORY, &saved_path);
		}
...
static int sdcardfs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	if (sbi->options.is_androidm) {
		...
		path = dentry_path_raw(dentry, ori_path, PATH_MAX);
	}
	err = mnt_want_write(lower_path.mnt);
	if (err)
		goto out_unlock;
...
out_unlock:
	...
	dput(lower_dentry);
	sdcardfs_put_lower_path(dentry, &lower_path);
	if (err == 0 && sbi->options.is_androidm)
		sdcardfs_notify_delete(sbi, path);
	if (sbi->options.is_androidm)
		__putname(ori_path);
	REVERT_CRED(saved_cred);
out_eacces:
...
static int sdcardfs_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	if (sbi->options.is_androidm && (!strcasecmp(dentry->d_name.name, "obb"))
		&& (pi->perm == PERM_ANDROID) && (pi->userid == 0))
		make_nomedia_in_obb = 1;
	else if (!sbi->options.is_androidm && (sbi->options.derive == DERIVE_UNIFIED) && (!strcasecmp(dentry->d_name.name, "obb"))
		&& (pi->perm == PERM_ANDROID) && (pi->userid == 0))
		make_nomedia_in_obb = 1;
			 ...
			 struct inode *new_dir, struct dentry *new_dentry)
{
	...
	if (sbi->options.is_androidm) {
		fix_derived_permission_androidm(sbi, new_dir);
	} else {
	...
	if (new_dir != old_dir) {
		...
		if (sbi->options.is_androidm) {
			fix_derived_permission_androidm(sbi, old_dir);
		} else {
		...
		if(new_parent) {
			if(old_dentry->d_inode) {
				...
				if (sbi->options.is_androidm) {
					fix_derived_permission_androidm(sbi, old_dentry->d_inode);
				} else {
		 ...
		 struct kstat *stat)
{
	 ...
	 */
	if (sbi->options.is_androidm) {
		fix_derived_permission_androidm(sbi, inode);
	} else {
