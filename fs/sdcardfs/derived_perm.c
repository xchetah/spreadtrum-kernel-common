...
/* modify uid/gid according to android permission control policy */
void fix_derived_permission_androidm(struct sdcardfs_sb_info* sbi, struct inode* inode)
{
	mode_t visible_mode = 0775 & ~(sbi->mask);

	inode->i_uid = SDCARDFS_I(inode)->d_uid;

	if (sbi->gid == AID_SDCARD_RW) {
		inode->i_gid = AID_SDCARD_RW;
	} else {
		inode->i_gid = multiuser_get_uid(SDCARDFS_I(inode)->userid, sbi->gid);
	}

	if (SDCARDFS_I(inode)->perm == PERM_PRE_ROOT) {
		visible_mode = 0711;
	} else if (SDCARDFS_I(inode)->under_android) {
		if (sbi->gid == AID_SDCARD_RW) {
	...
	inode->i_mode = (inode->i_mode & S_IFMT) | visible_mode;
}
...
void setup_derived_state_androidm(struct inode *inode, perm_t perm,
...
void get_derived_permission(struct dentry *parent, struct dentry *dentry)
{
	...
	if ( !sbi->options.is_androidm && sbi->options.derive == DERIVE_NONE) {
		return;
	}
	...
	case PERM_ANDROID:
		if (!strcasecmp(dentry->d_name.name, "data")) {
		...
		} else if (!sbi->options.is_androidm && !strcasecmp(dentry->d_name.name, "user")) {
			...
			info->d_mode = 00770;
		} else if (sbi->options.is_androidm && !strcasecmp(dentry->d_name.name, "media")) {
			/* App-specific directories inside; let anyone traverse */
...
inline void update_derived_permission(struct dentry *dentry)
{
	...
	if (sbi->options.is_androidm) {
		fix_derived_permission_androidm(sbi, dentry->d_inode);
	} else {
...
int need_graft_path(struct dentry *dentry)
{
	...
	if(parent_info->perm == PERM_ANDROID &&
		...
		if (sbi->options.is_androidm) {
			if(!(!sbi->options.multi_user && parent_info->userid == 0)) {
				ret = 1;
			}
		} else {
