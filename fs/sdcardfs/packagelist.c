...
void * packagelist_create(gid_t write_gid, bool is_androidm)
{
	...
	if (!is_androidm) {
		...
		pkgl_dat->is_androidm = false;
	} else {
		pkgl_dat->is_androidm = true;
		pkgl_dat->count = 1;
	}
...
void packagelist_destroy(void *pkgl_id)
{
	...
	/*only do actual destroy action after three umount cmd*/
	if( pkgl_dat->is_androidm && pkgl_dat->count < 3){
			pkgl_dat->count++;
			printk(KERN_INFO "sdcardfs count=%d\n",pkgl_dat->count);
			return;
    }
