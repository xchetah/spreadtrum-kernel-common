...
#include <linux/input/bt432_ts_j1.h>
...
u8* m_firmware_data;
#else
#include "zinitix_touch_bt432_j1_firmware.h"
#endif
...
static void run_reference_HFDND(void *device_data)
{
	...
	if (temp) {
		ret_string = "HF DND pass";
		finfo->cmd_state = OK;
	} else {
		ret_string = "HF DND Fail";
		finfo->cmd_state = FAIL;
	}

	pr_info("%s", ret_string);
	sprintf(finfo->cmd_buff, "%d,%d\n", info->hfdnd_real_min_val, info->hfdnd_real_max_val);
	set_cmd_result(info, finfo->cmd_buff, strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
}
