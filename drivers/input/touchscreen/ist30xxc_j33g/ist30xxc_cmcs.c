...
int ist30xx_cmcs_test(struct ist30xx_data *data, const u8 *buf, int size)
{
	...
	/* Read CM data */
	memset(ts_cmcs_buf->cm, 0, tsp->node->len * sizeof(s16));
	...
	/* Read CS data */
	memset(ts_cmcs_buf->cs, 0, tsp->node->len * sizeof(s16));
...
int ist30xx_init_cmcs_sysfs(struct ist30xx_data *data)
{
	...
	ts_cmcs_buf->cm = kmalloc(pdata->max_node * sizeof(s16), GFP_KERNEL);
	ts_cmcs_buf->spec_min = kmalloc(pdata->max_node * sizeof(s16), GFP_KERNEL);
	ts_cmcs_buf->spec_max = kmalloc(pdata->max_node * sizeof(s16), GFP_KERNEL);
	ts_cmcs_buf->slope0 = kmalloc(pdata->max_node * sizeof(s16), GFP_KERNEL);
	ts_cmcs_buf->slope1 = kmalloc(pdata->max_node * sizeof(s16), GFP_KERNEL);
	ts_cmcs_buf->cs = kmalloc(pdata->max_node * sizeof(s16), GFP_KERNEL);
	...
	ret = ist30xx_get_cmcs_info(ts_cmcs_bin, ts_cmcs_bin_size);
	if (unlikely(ret))
		tsp_warn("istxx_init_cmcs_sysfs ret = %d \n", ret);
#endif
