...
#elif defined(CONFIG_MACH_SP9830I_J3LTE) || defined(CONFIG_MACH_TABELTE) || defined(CONFIG_MACH_SP9830I_J2LTE) || defined(CONFIG_MACH_SHARKLS_J3VLT)
#include "./firmware/j3xlte_fw.h"
#else
...
int ist30xxc_isp_mode(struct i2c_client *client, int mode)
{
	...
	if (data->pdata->chip_code > IMAGIS_IST3032C)
	val &= ~(0x8000);
...
int ist30xxc_read_chksum_all(struct i2c_client *client, u32 *chksum)
{
	...
	end_addr = IST30XX_FLASH_BASE_ADDR + total_size;
...
#if I2C_BURST_MODE
	for (i = 0; i < total_size; i += I2C_MAX_READ_SIZE) {
		len = I2C_MAX_READ_SIZE / IST30XX_DATA_LEN;
		if ((total_size - i) < I2C_MAX_READ_SIZE)
			len = (total_size - i) / IST30XX_DATA_LEN;
...
int ist30xxc_isp_fw_update(struct ist30xx_data *data, const u8 *buf)
{
	...
	ret = ist30xxc_isp_program(data->client, addr, IST30XX_ISP_PROG_TOTAL_B,
					(u32 *)buf, total_size / IST30XX_DATA_LEN);
	if (unlikely(ret))
		goto isp_fw_update_end;
#endif
#else
...
{
	if(data->pdata->chip_ic < IMAGIS_IST3038C){
	char *str;
	struct ist30xx_fw *fw = &data->fw;
				...
				struct device_attribute *attr, char *buf)
{
...
#if IST30XX_INTERNAL_BIN
	{
		...
		if (unlikely(ret)) {
			tsp_warn("ist30xx_fw_version_show ret = %d\n", ret);
		} else {
