...
#define CTP_SET_INCALL_STATE

#ifdef CTP_SET_INCALL_STATE
#define CTP_INCALL_PROC_FILE			"incall_state"
static struct proc_dir_entry *ctp_incall_proc = NULL;
#endif
...
static void ft5x0x_clear_report_data(struct ft5x0x_ts_data *ft5x0x_ts)
{
	int i;
		...
		if(last_key_code != 0xff)
		{
			...
			input_report_key(ft5x0x_ts->input_dev, last_key_code, 0);
...
static int ft5x0x_update_data(void)
{
	...
	for(i = 0; i < TS_MAX_FINGER; i++)
	{
		...
		if((buf[6*i+3] & 0x40) == 0x0)
		{
			...
			if(y >= key_ystart)
			{
				...
				if(last_key_code == 0xff)
				{
					//printk("ft5x0x_update_data  x%d = %d,y%d = %d, key_code=%d\n",i, x, i, y, key_code);
	...
	if(0 == event->touch_point)
	{
		...
		if(last_key_code != 0xff)
		{
			...
			input_report_key(data->input_dev, last_key_code, 0);
#if !MULTI_PROTOCOL_TYPE_B
			input_mt_sync(data->input_dev);
#endif
...
static ssize_t tp_version_read(struct file *f, char __user *buf, size_t count, loff_t *pos)
{
	...
	if (err < 0)
	{
		printk("[FST] read FW version error %x\n", minor);
		return -EFAULT;
	}

	ver_no = minor;
	if(copy_to_user(buf, (void *)&ver_no, sizeof(ver_no)))
	{
		return -EFAULT;
	}
...
#ifdef CTP_SET_INCALL_STATE
static ssize_t ctp_incall_write_proc(struct file *f, char __user *buf, size_t count, loff_t *pos)
{
	int incall_state = 0;

	if(copy_from_user(&incall_state, buf, sizeof(incall_state)))
	{
		printk("__ctp_incall_write_proc__: call copy_from_user() fail!!\r\n");
		return -EFAULT;
	}

	printk("__ctp_incall_write_proc__: incall_state = 0x%x\r\n", incall_state);

	incall_state &= 0x1;

	if(incall_state)
	{
		ft5x0x_write_reg(0xad, 0x1);
	}
	else
	{
		ft5x0x_write_reg(0xad, 0x0);
	}

	return count;
}

static const struct file_operations ctp_incall_proc_fops =
{ 
	.write = ctp_incall_write_proc
};
#endif
...
static int ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("ft5x0x_ts_probe   ft5x0x_ts_pen_irq_work\n");
...
#endif
	printk("ft5x0x_ts_probe  focaltech_get_upgrade_array \n");
	...
	{
		printk("[FST] tp_version_struct register failed\n");
	}
#endif
	...
	ctp_incall_proc = proc_create(CTP_INCALL_PROC_FILE, 0222, NULL, &ctp_incall_proc_fops);
	if (ctp_incall_proc == NULL)
	{
		printk("create_proc_entry %s failed\n", CTP_INCALL_PROC_FILE);
	}
