...
tsp_pinmap_t tsp_rstpin_map_HWic[] = {
	{REG_PIN_SPI0_CLK, BIT_PIN_SLP_AP|BIT_PIN_WPUS|BITS_PIN_DS(1)|BITS_PIN_AF(1)|BIT_PIN_WPU|BIT_PIN_SLP_WPU|BIT_PIN_SLP_Z},
	{REG_PIN_SPI0_DO, BIT_PIN_SLP_AP|BIT_PIN_WPUS|BITS_PIN_DS(1)|BITS_PIN_AF(1)|BIT_PIN_WPU|BIT_PIN_SLP_WPU|BIT_PIN_SLP_Z},
};

tsp_pinmap_t tsp_rstpin_map_gpio[] = {
	{REG_PIN_SPI0_CLK, BIT_PIN_SLP_AP|BIT_PIN_WPUS|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPU|BIT_PIN_SLP_WPU|BIT_PIN_SLP_Z},
	{REG_PIN_SPI0_DO, BIT_PIN_SLP_AP|BIT_PIN_WPUS|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPU|BIT_PIN_SLP_WPU|BIT_PIN_SLP_Z},
};
...
void mip_gpio_mode(struct mip_ts_info *info, bool mode)
{
	if(mode){	  ////Set GPIO mode 
		sci_glb_write(CTL_PIN_BASE + tsp_rstpin_map_gpio[0].reg,
				tsp_rstpin_map_gpio[0].val, 0xffffffff);
		sci_glb_write(CTL_PIN_BASE + tsp_rstpin_map_gpio[1].reg,
				tsp_rstpin_map_gpio[1].val, 0xffffffff);

	}
	else { //Set I2C mode
		sci_glb_write(CTL_PIN_BASE + tsp_rstpin_map_HWic[0].reg,
				tsp_rstpin_map_HWic[0].val, 0xffffffff);
		sci_glb_write(CTL_PIN_BASE + tsp_rstpin_map_HWic[1].reg,
				tsp_rstpin_map_HWic[1].val, 0xffffffff);
...
int mip_flash_fw(struct mip_ts_info *info, const u8 *fw_data, size_t fw_size, bool force, bool section)
{
	...
	if (force)
		...
		if (retry < 0)
			...
			dev_info(&client->dev, "Chip firmware version [0x%04X]\n", ver_chip[2]);
	...
	if (!bin_data) {
		dev_err(&client->dev, "failed to allocate bin_data\n");
		ret = fw_err_download;
		goto ERROR;
	}
