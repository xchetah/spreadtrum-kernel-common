...
//								key, key_code, key_state);
				...
				dev_err(&info->client->dev, "Unknown key code [%d]\n", key);
				continue;
			}
		} else
		{
...
int mip_parse_devicetree(struct device *dev, struct melfas_platform_data *pdata)
{
	...
	ret  = of_property_read_string(np, MIP_DEVICE_NAME",fw_name", &pdata->fw_name);
	if (ret < 0) {
		dev_err(dev, "failed to get fw_name\n");
		return -EINVAL;
	}
	ret = of_property_read_string(np, MIP_DEVICE_NAME",ext_fw_name", &pdata->ext_fw_name);
	if (ret < 0) {
		dev_err(dev, "failed to get ext_fw_name\n");
		pdata->ext_fw_name = "/sdcard/melfas_mip.bin";
	}
	...
	ret = gpio_request(pdata->gpio_intr, "mip4_intr");
	if (ret < 0) {
	...
	ret = gpio_request(pdata->gpio_scl , "mip4_scl");
	if (ret < 0) {
	...
	ret = gpio_request(pdata->gpio_sda, "mip4_sda");
	if (ret < 0) {
