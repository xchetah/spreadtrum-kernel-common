...
int mip_flash_fw(struct mip_ts_info *info, const u8 *fw_data, size_t fw_size, bool force, bool section)
{
			...
			if(section == false){
				...
				if(ver_chip[0] != bin_info->ver_boot){
					if((bin_info->boot_end - bin_info->boot_start) > 0){
						...
						dev_err(&client->dev, "%s [ERROR] wrong offset\n", __func__);
						ret = fw_err_file_type;
						goto ERROR_FILE;
					}
				}
	...
	if(offset_start == 0){
		full_download = true;
		dev_dbg(&client->dev, "%s - Full download\n", __func__);
	}
	else{
		full_download = false;
		dev_dbg(&client->dev, "%s - Section download\n", __func__);
	}
...
ERROR_UPDATE:
	dev_err(&client->dev, "%s [ERROR_UPDATE]\n", __func__);
	kfree(bin_data);
...
ERROR_FILE: 
	dev_err(&client->dev, "%s [ERROR_FILE]\n", __func__);
