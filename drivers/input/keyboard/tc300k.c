...
struct tsk_event_val tsk_ev[4] = {
		{0x01 << 0, TSK_PRESS, KEY_APPSELECT, "recent"},
		{0x01 << 1, TSK_PRESS,  KEY_BACK, "back"},
		{0x01 << 4, TSK_RELEASE, KEY_APPSELECT, "recent"},
		{0x01 << 5, TSK_RELEASE, KEY_BACK, "back"},
};
#else
struct tsk_event_val tsk_ev[4] = {
		{0x01 << 0, TSK_PRESS, KEY_BACK, "recent"},
		{0x01 << 1, TSK_PRESS,  KEY_APPSELECT, "back"},
		{0x01 << 4, TSK_RELEASE, KEY_BACK, "recent"},
		{0x01 << 5, TSK_RELEASE, KEY_APPSELECT, "back"},
};
#endif
struct tc300k_data {
...
#endif
static int tc300k_keycodes[] = {KEY_APPSELECT,KEY_BACK};
#if USE_PIN_CTRL
...
#ifdef CONFIG_MACH_TABELTE  
	struct i2c_client *client = data->client;
	static struct regulator *tk_vdd;
	int ret;

	tk_vdd = regulator_get(&client->dev, "vddkpled");

	if (IS_ERR(tk_vdd)) {
		dev_err(&client->dev, "TK regulator_get error!\n");
		return;
	}
       if( regulator_set_voltage(tk_vdd ,2200000,2200000))return;
...
static irqreturn_t tc300k_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0 ; i < data->key_num * 2 ; i++) {
		...
		if (key_handle_flag) {
				input_report_key(data->input_dev, data->tsk_ev_val[i].tsk_keycode, data->tsk_ev_val[i].tsk_status);
...
static int tc300k_write_fw(struct tc300k_data *data)
{
	...
	udelay(1);
	printk("%s 2>>>>> addr:%X, FW len: %X \n", __func__, addr, data->fw_img->fw_len);
	while (addr < data->fw_img->fw_len) {
		code_data = data->fw_img->data[addr++];
		load(data, code_data);
		usleep_range(20, 21);/*printk("%s 3 addr:%X, FW len: %X \n", __func__, addr, data->fw_img->fw_len);*/
	}
	printk("%s 4\n", __func__);
...
write_fw:
	...
	printk("%s Verify done \n", __func__);

	tc300k_reset_for_isp(data, false);
	printk("%s ISP reset done \n", __func__);
...
static int tc300k_flash_fw(struct tc300k_data *data, u8 fw_path, bool force)
{
...
#if 1
	if (fw_ver == 6) {
		dev_info(&client->dev, "forced fw write!\n");
		force = true;
	}
#endif
#ifdef LED_TWINKLE_BOOTING
	...
	printk("%s module vers:%d \n", __func__, module_ver);
#if 0 /* For Force Update*/
	if (!data->pdata->firmup) {
		dev_info(&client->dev, "pass firmup, h/w rev not support\n");
		ret = HAVE_LATEST_FW;
		printk("%s Failed at HW latest fw check\n", __func__);
		goto out;
	}
#endif
	printk("%s 4 \n", __func__);
...
static int tc300k_initialize(struct tc300k_data *data)
{
	...
	ret = tc300k_flash_fw(data, FW_BUILT_IN, true);
	printk("%s 2 \n", __func__);
	if (ret < 0)
...
static int tc300k_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		...
		printk("%s Failed at Initialize FW \n", __func__);
		goto err_initialize;
	}
#ifdef USE_TKEY_UPDATE_WORK
	switch (ret) {
