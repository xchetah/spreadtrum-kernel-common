...
*/
static void cmd_clear_result(struct mip4_tk_info *info)
{
	char delim = ':';
...
* Set command result
*/
static void cmd_set_result(struct mip4_tk_info *info, char *buf, int len)
{
...
{
	struct mip4_tk_info *info = (struct mip4_tk_info *)device_data;
	char buf[64] = { 0 };
...
static void cmd_get_fw_ver_bin(void *device_data)
{
	...
	if(mip_get_fw_version_from_bin(info, ver_file)){
		sprintf(buf, "%s", "NG");
		info->cmd_state = CMD_STATUS_FAIL;
		goto EXIT;
	}
...
static ssize_t mip_sys_cmd(struct device *dev, struct device_attribute *devattr, const char *buf, size_t count)
{
	...
	if (!info) {
		dev_err(&info->client->dev, "%s [ERROR] mip4_tk_info not found\n", __func__);
		ret = -EINVAL;
		goto ERROR;
	}
...
*/
int mip_sysfs_cmd_create(struct mip4_tk_info *info)
{
	...
	*/
	info->cmd_dev = device_create(sec_class, NULL, 0, info, "touchkey");
...
*/
void mip_sysfs_cmd_remove(struct mip4_tk_info *info)
{
