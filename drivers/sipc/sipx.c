                          ...
                          int *yell, int *full)
{
        ...
        pos = sblock_get_ringpos(ring->fifo_info->fifo_wrptr, ring->fifo_size);
                                ...
                                struct sipx_channel *sipx_chan)
{
        ...
        while(fifo_info->fifo_wrptr != fifo_info->fifo_rdptr) {
                ...
                SIPX_INFO("sipx_recover_channel dl_ring fifo_info:%x, wrptr:%x, rdptr:%x\n",
                          fifo_info, fifo_info->fifo_wrptr, fifo_info->fifo_rdptr);

                pos = sblock_get_ringpos(fifo_info->fifo_rdptr, sipx_chan->dl_ring->fifo_size);
        ...
        while(fifo_info->fifo_wrptr != fifo_info->fifo_rdptr) {
                ...
                SIPX_INFO("sipx_recover_channel dl_ack_ring fifo_info:%x, wrptr:%x, rdptr:%x\n",
                          fifo_info, fifo_info->fifo_wrptr, fifo_info->fifo_rdptr);

                pos = sblock_get_ringpos(fifo_info->fifo_rdptr, sipx_chan->dl_ack_ring->fifo_size);
        ...
        while(fifo_info->fifo_wrptr != fifo_info->fifo_rdptr) {
                ...
                SIPX_INFO("sipx_recover_channel ul_ring fifo_info:%x, wrptr:%x, rdptr:%x\n",
                          fifo_info, fifo_info->fifo_wrptr, fifo_info->fifo_rdptr);

                pos = sblock_get_ringpos(fifo_info->fifo_rdptr, sipx_chan->ul_ring->fifo_size);
        ...
        while(fifo_info->fifo_wrptr != fifo_info->fifo_rdptr) {
                ...
                SIPX_INFO("sipx_recover_channel ul_ack_ring fifo_info:%x, wrptr:%x, rdptr:%x\n",
                          fifo_info, fifo_info->fifo_wrptr, fifo_info->fifo_rdptr);

                pos = sblock_get_ringpos(fifo_info->fifo_rdptr, sipx_chan->ul_ack_ring->fifo_size);
...
static int sipx_thread(void *data)
{
        ...
        while (!kthread_should_stop()) {
                ...
                case SMSG_TYPE_CLOSE:
                        ...
                        if (sipx_chan->handler) {
                                sipx_chan->handler(SBLOCK_NOTIFY_CLOSE, sipx_chan->data);
                        }
                        sipx_chan->state = SBLOCK_STATE_IDLE;
                        break;
                case SMSG_TYPE_CMD:
                        ...
                        if (sipx_chan->sipx->recovery && (mrecv.value == 0)) {
                                SIPX_INFO("sipx_chan start recover! recv msg: dst=%d, channel=%d, "
                                          "type=%d, flag=0x%04x, value=0x%08x\n",
                                          sipx_chan->dst, sipx_chan->channel,
                                          mrecv.type, mrecv.flag, mrecv.value);
                                if (sipx_chan->handler) {
                        ...
                        smsg_set(&mcmd, sipx_chan->channel, SMSG_TYPE_DONE,
                                 SMSG_DONE_SBLOCK_INIT, sipx_chan->sipx->smem_addr);
                        smsg_send(sipx_chan->dst, &mcmd, -1);
                        if (sipx_chan->handler) {
                                sipx_chan->handler(SBLOCK_NOTIFY_OPEN, sipx_chan->data);
                        }
                ...
                case SMSG_TYPE_EVENT:
                        ...
                        case SMSG_EVENT_SBLOCK_SEND:
                                if (sipx_chan->handler) {
                                        sipx_chan->handler(SBLOCK_NOTIFY_RECV, sipx_chan->data);
                                }
                                break;
...
#endif
                                if (sipx_chan->handler) {
                                        sipx_chan->handler(SBLOCK_NOTIFY_GET, sipx_chan->data);
                                }
                                break;
                        default:
                                rval = 1;
        ...
        SIPX_ERR("sipx_chan %d-%d thread stop", sipx_chan->dst, sipx_chan->channel);
...
int destroy_spix_mgr(struct sipx_mgr *sipx)
{
        ...
        if(sipx->smem_virt) {
                iounmap(sipx->smem_virt);
        }
#ifdef CONFIG_SIPCMEM_CACHE_EN

        if(sipx->smem_cached_virt) {
                iounmap(sipx->smem_cached_virt);
        }
#endif
        if(sipx->smem_addr) {
...
int create_sipx_mgr(struct sipx_mgr **out ,struct sipx_init_data *pdata)
{
        ...
        total_size = sizeof(struct sipx_fifo_info);                          /* dl pool info */
        total_size += sizeof(struct sipx_fifo_info);                         /* dl ack pool info */
        total_size += sizeof(struct sipx_fifo_info);                         /* ul pool info */
        total_size += sizeof(struct sipx_fifo_info);                         /* ul ack pool info */
        offset_chan_cnt = total_size;
        total_size += sizeof(uint32_t) * (SIPX_CH_NR + 1);                   /* channels smem ptr */
        offset_dl_blk = total_size;
        total_size += sipx->dl_pool_size * sipx->blk_size;                      /* dl blk mem */
        offset_dl_ack_blk = total_size;
        total_size += sipx->dl_ack_pool_size * sipx->ack_blk_size;              /* dl ack blk mem */
        offset_ul_blk = total_size;
        total_size += sipx->ul_pool_size * sipx->blk_size;                      /* ul blk mem */
        offset_ul_ack_blk = total_size;
        total_size += sipx->ul_ack_pool_size * sipx->ack_blk_size;              /* ul ack blk mem */
        offset_dl_fifo = total_size;
        total_size += sizeof(struct sipx_blk_item) * sipx->dl_pool_size;                /* dl pool fifo mem */
        offset_dl_ack_fifo = total_size;
        total_size += sizeof(struct sipx_blk_item) * sipx->dl_ack_pool_size;            /* dl ack pool fifo mem */
        offset_ul_fifo = total_size;
        total_size += sizeof(struct sipx_blk_item) * sipx->ul_pool_size;                /* ul pool fifo mem */
        offset_ul_ack_fifo = total_size;
        total_size += sizeof(struct sipx_blk_item) * sipx->ul_ack_pool_size;            /* ul ack pool fifo mem */
        ...
        sipx->smem_virt = ioremap_nocache(sipx->smem_addr, sipx->smem_size);
        if (!sipx->smem_virt) {
        ...
        sipx->smem_cached_virt = ioremap_cache(sipx->smem_addr, sipx->smem_size);
        if (!sipx->smem_cached_virt) {
        ...
        fifo_info->blks_addr = sipx->smem_addr + offset_dl_blk; /* offset of smem_addr */
        fifo_info->fifo_size = sipx->dl_pool_size;
        fifo_info->blk_size = sipx->blk_size;
        fifo_info->fifo_rdptr = 0;
        fifo_info->fifo_wrptr = 0;
        fifo_info->fifo_addr = sipx->smem_addr + offset_dl_fifo;  /* offset of smem_addr */
#ifdef CONFIG_SIPCMEM_CACHE_EN

        ret = create_sipx_pool_ctrl(&sipx->dl_pool, fifo_info,
...
#ifdef CONFIG_SIPCMEM_CACHE_EN
        ret = create_sipx_pool_ctrl(&sipx->dl_ack_pool, fifo_info,
        ...
        fifo_info->blks_addr = sipx->smem_addr + offset_ul_blk; /* offset of smem_addr */
        fifo_info->fifo_size = sipx->ul_pool_size;
        fifo_info->blk_size = sipx->blk_size;
        fifo_info->fifo_rdptr = 0;
        fifo_info->fifo_wrptr = 0;
        fifo_info->fifo_addr = sipx->smem_addr + offset_ul_fifo;  /* offset of smem_addr */
#ifdef CONFIG_SIPCMEM_CACHE_EN
        ...
        ret = create_sipx_pool_ctrl(&sipx->ul_ack_pool, fifo_info,
...
static int sipx_parse_dt(struct sipx_init_data **init, struct device *dev)
{
        ...
        pdata = kzalloc(sizeof(struct sipx_init_data), GFP_KERNEL);
        if (!pdata) {
                return -ENOMEM;
        }
        ...
        ret = of_property_read_u32(np, "sprd,dl-pool", (uint32_t *)&pdata->dl_pool_size);
        if (ret) {
                goto error;
        }

        ret = of_property_read_u32(np, "sprd,dl-ack-pool", (uint32_t *)&pdata->dl_ack_pool_size);
        if (ret) {
                goto error;
        }

        ret = of_property_read_u32(np, "sprd,ul-pool", (uint32_t *)&pdata->ul_pool_size);
        if (ret) {
                goto error;
        }

        ret = of_property_read_u32(np, "sprd,ul-ack-pool", (uint32_t *)&pdata->ul_ack_pool_size);
        if (ret) {
                goto error;
        }
...
int sipx_get(uint8_t dst, uint8_t channel, struct sblock *blk, int is_ack)
{
        ...
        sipx_chan = sipx->channels[channel];
        ...
        if(is_ack) {
                ...
                if(ret) {
                        SIPX_INFO("sipx_get %d-%d ack pool is empty!\n",
                                  dst, channel);
                        /* ack packet, when  ack pool is empty, try normal pool */
...
{
        struct sipx_channel *sipx_chan = container_of(timer, struct sipx_channel,
                                         ul_timer);
        struct smsg mevt;
        unsigned long flags;
        ...
        smsg_set(&mevt, sipx_chan->channel, SMSG_TYPE_EVENT, SMSG_EVENT_SBLOCK_SEND, 0);
...
int sipx_send(uint8_t dst, uint8_t channel, struct sblock *blk)
{
        ...
        SIPX_DEBUG("seblock_send: dst=%d, channel=%d, addr=%p, len=%d\n",
                            ...
                            struct sipx_channel *sipx_chan)
{
        ...
        fifo_info->fifo_rdptr = 0;
        fifo_info->fifo_wrptr = 0;
        offset = sizeof(struct sipx_fifo_info) * 4;                  /* all ring fifo info */
        ...
        fifo_info = (volatile struct sipx_fifo_info *)(sipx_chan->smem_virt +
                        sizeof(struct sipx_fifo_info));                       /* dl ring info */
        fifo_info->blks_addr = 0;
        fifo_info->blk_size = sipx->ack_blk_size;
        fifo_info->fifo_size = sipx->dl_ack_pool_size;
        fifo_info->fifo_rdptr = 0;
        fifo_info->fifo_wrptr = 0;
        offset = (sizeof(struct sipx_fifo_info) * 4) +               /* dl ul ring pool info */
                 (sizeof(struct sipx_blk_item) * sipx->dl_pool_size);     /* dl ring fifo mem */
        fifo_info->fifo_addr = sipx_chan->smem_addr + offset;
        ret = create_sipx_ring_ctrl(&sipx_chan->dl_ack_ring, fifo_info,
                                    sipx_chan->smem_virt + offset,
                                    sipx->dl_ack_pool->blks_buf,sipx);
        if(ret) {
                return ret;
        }
        /* ul ring fifo info */
        fifo_info = (volatile struct sipx_fifo_info *)(sipx_chan->smem_virt +
                        sizeof(struct sipx_fifo_info) +                      /* dl ring info */
                        sizeof(struct sipx_fifo_info));                      /* ul ring info */
        fifo_info->blks_addr = 0;
        fifo_info->blk_size = sipx->blk_size;
        fifo_info->fifo_size = sipx->ul_pool_size;
        fifo_info->fifo_rdptr = 0;
        ...
        offset = (sizeof(struct sipx_fifo_info) * 4) +                       /* dl ul ring pool info */
                 (sizeof(struct sipx_blk_item) * sipx->dl_pool_size) +            /* dl ring fifo mem */
                 (sizeof(struct sipx_blk_item) * sipx->dl_ack_pool_size);         /* ul ring fifo mem */
        fifo_info->fifo_addr = sipx_chan->smem_addr + offset;
        ret = create_sipx_ring_ctrl(&sipx_chan->ul_ring, fifo_info,
                                    sipx_chan->smem_virt + offset,
                                    sipx->ul_pool->blks_buf,sipx);
        if(ret) {
                return ret;
        }
        ...
        fifo_info = (volatile struct sipx_fifo_info *)(sipx_chan->smem_virt +
                        sizeof(struct sipx_fifo_info) +                      /* dl ring info */
                        sizeof(struct sipx_fifo_info) +                      /* dl ack ring info */
                        sizeof(struct sipx_fifo_info));                      /* ul ring info */
        fifo_info->blks_addr = 0;
        fifo_info->blk_size = sipx->ack_blk_size;
        fifo_info->fifo_size = sipx->ul_ack_pool_size;;
        ...
        offset = (sizeof(struct sipx_fifo_info) * 4) +                       /* dl ul ring pool info */
                 (sizeof(struct sipx_blk_item) * sipx->dl_pool_size) +            /* dl ring fifo mem */
                 (sizeof(struct sipx_blk_item) * sipx->dl_ack_pool_size) +        /* ul ring fifo mem */
                 (sizeof(struct sipx_blk_item) * sipx->ul_pool_size);             /* dl pool fifo mem */
        fifo_info->fifo_addr = sipx_chan->smem_addr + offset;
        ret = create_sipx_ring_ctrl(&sipx_chan->ul_ack_ring, fifo_info,
                                    sipx_chan->smem_virt + offset,
                                    sipx->ul_ack_pool->blks_buf,sipx);
                            ...
                            struct sipx_channel *sipx_chan)
{
        ...
        offset = sizeof(struct sipx_fifo_info) * 4;                          /* all pool info */
                             ...
                             struct sipx_channel **out)
{
        ...
        sipx_chan->sipx = sipx;
        sipx_chan->smem_size = sizeof(struct sipx_fifo_info) * 4;      	        /* dl ul ring pool info */
        sipx_chan->smem_size += sizeof(struct sipx_blk_item) * sipx->dl_pool_size;        /* dl ring fifo mem */
        sipx_chan->smem_size += sizeof(struct sipx_blk_item) * sipx->dl_ack_pool_size;    /* dl ack ring fifo mem */
        sipx_chan->smem_size += sizeof(struct sipx_blk_item) * sipx->ul_pool_size;        /* ul ring fifo mem */
        sipx_chan->smem_size += sizeof(struct sipx_blk_item) * sipx->ul_ack_pool_size;    /* ul ack ring fifo mem */

        SIPX_DEBUG("sipx_chan->smem_size = %d\n", sipx_chan->smem_size);

        sipx_chan->smem_addr = smem_alloc(sipx_chan->smem_size);		/*single channel in smem*/
        if (!sipx_chan->smem_addr) {
                SIPX_ERR("Failed to allocate smem for sipx_chan\n");
                kfree(sipx_chan);
                return -ENOMEM;
        }
        sipx_chan->smem_virt = ioremap_nocache(sipx_chan->smem_addr, sipx_chan->smem_size);
        if (!sipx_chan->smem_virt) {
        ...
        update_sipx_channel_ptr(sipx, sipx_chan);		/*physical address*/
        sipx->channels[channel] = sipx_chan;
...
int sipx_chan_create(uint8_t dst, uint8_t channel)
{
        ...
        BUG_ON((dst >= SIPC_ID_NR) || (channel >= SIPX_CH_NR));
...
static int sipx_probe(struct platform_device *pdev)
{
        ...
        SIPX_INFO("sipx device tree, name=%s, dst=%u, pool size=%d, %d, %d, %d\n",
                  pdata->name, pdata->dst, pdata->dl_pool_size, pdata->dl_ack_pool_size,
                  pdata->ul_pool_size, pdata->ul_ack_pool_size);
...
static int sipx_debug_show(struct seq_file *m, void *private)
{
        ...
        for (i = 0; i < SIPC_ID_NR; i++) {
                ...
                seq_printf(m, "sipx dst 0x%0x, state: 0x%0x, recovery: %d, smem_virt: 0x%0x, smem_cached_virt: 0x%x, smem_addr: 0x%0x, smem_size: 0x%0x\n",
                           sipx->dst, sipx->state, sipx->recovery,
                           (size_t)sipx->smem_virt, (size_t)sipx->smem_cached_virt,sipx->smem_addr,sipx->smem_size);
#else
                seq_printf(m, "sipx dst 0x%0x, state: 0x%0x, recovery: %d, smem_virt: 0x%0x, smem_addr: 0x%0x, smem_size: 0x%0x\n",
                        sipx->dst, sipx->state, sipx->recovery,
                        (size_t)sipx->smem_virt, sipx->smem_addr,sipx->smem_size);
#endif
                seq_printf(m, "dl_pool_size: %d, dl_ack_pool_size: %d, ul_pool_size: %d, ul_ack_pool_size: %d, blk_size: %d, ack_blk_size: %d\n",
                           sipx->dl_pool_size, sipx->dl_ack_pool_size,
                           sipx->ul_pool_size, sipx->ul_ack_pool_size,
                           sipx->blk_size, sipx->ack_blk_size);

                fifo_info = sipx->dl_pool->fifo_info;
                seq_printf(m, "dl_pool: blks_addr :0x%0x, blk_size :%d, fifo_addr :0x%0x, fifo_size :%d, fifo_rdptr :0x%0x, fifo_wrptr: 0x%0x \n",
                ...
                seq_printf(m, "dl_ack_pool: blks_addr :0x%0x, blk_size :%d, fifo_addr :0x%0x, fifo_size :%d, fifo_rdptr :0x%0x, fifo_wrptr: 0x%0x \n",
                           (uint32_t)fifo_info->blks_addr, (uint32_t)fifo_info->blk_size,
                           (uint32_t)fifo_info->fifo_addr, (uint32_t)fifo_info->fifo_size,
                           (uint32_t)fifo_info->fifo_rdptr, (uint32_t)fifo_info->fifo_wrptr);
                fifo_info = sipx->ul_pool->fifo_info;
                seq_printf(m, "ul_pool: blks_addr :0x%0x, blk_size :%d, fifo_addr :0x%0x, fifo_size :%d, fifo_rdptr :0x%0x, fifo_wrptr: 0x%0x \n",
                           (uint32_t)fifo_info->blks_addr, (uint32_t)fifo_info->blk_size,
                           (uint32_t)fifo_info->fifo_addr, (uint32_t)fifo_info->fifo_size,
                           (uint32_t)fifo_info->fifo_rdptr, (uint32_t)fifo_info->fifo_wrptr);
                fifo_info = sipx->ul_ack_pool->fifo_info;
                seq_printf(m, "ul_ack_pool: blks_addr :0x%0x, blk_size :%d, fifo_addr :0x%0x, fifo_size :%d, fifo_rdptr :0x%0x, fifo_wrptr: 0x%0x \n",
                           (uint32_t)fifo_info->blks_addr, (uint32_t)fifo_info->blk_size,
                           (uint32_t)fifo_info->fifo_addr, (uint32_t)fifo_info->fifo_size,
                           (uint32_t)fifo_info->fifo_rdptr, (uint32_t)fifo_info->fifo_wrptr);
                seq_printf(m, "************************************************************************************************\n");
                ...
                for (j=0;  j < SMSG_CH_NR; j++) {
                        ...
                        /* sipx channel */
                        seq_printf(m, "sipx_chan dst 0x%0x, channel: 0x%0x, state: %d, smem_virt: 0x%0x, smem_addr: 0x%0x, smem_size: 0x%0x, dl_record: 0x%0x, ul_record: 0x%0x \n",
                                   sipx_chan->dst, sipx_chan->channel, sipx_chan->state,
                                   (size_t)sipx_chan->smem_virt, sipx_chan->smem_addr,
                                   sipx_chan->smem_size, sipx_chan->dl_record_flag, sipx_chan->ul_record_flag);


                        fifo_info = sipx_chan->dl_ring->fifo_info;
                        seq_printf(m, "dl_ring: blks_addr :0x%0x, blk_size :%d, fifo_addr :0x%0x, fifo_size :%d, fifo_rdptr :0x%0x, fifo_wrptr: 0x%0x \n",
                                   (uint32_t)fifo_info->blks_addr, (uint32_t)fifo_info->blk_size,
                                   (uint32_t)fifo_info->fifo_addr, (uint32_t)fifo_info->fifo_size,
                                   (uint32_t)fifo_info->fifo_rdptr, (uint32_t)fifo_info->fifo_wrptr);
                        fifo_info = sipx_chan->dl_ack_ring->fifo_info;
                        seq_printf(m, "dl_ack_ring: blks_addr :0x%0x, blk_size :%d, fifo_addr :0x%0x, fifo_size :%d, fifo_rdptr :0x%0x, fifo_wrptr: 0x%0x \n",
                                   (uint32_t)fifo_info->blks_addr, (uint32_t)fifo_info->blk_size,
                                   (uint32_t)fifo_info->fifo_addr, (uint32_t)fifo_info->fifo_size,
                                   (uint32_t)fifo_info->fifo_rdptr, (uint32_t)fifo_info->fifo_wrptr);
                        fifo_info = sipx_chan->ul_ring->fifo_info;
                        seq_printf(m, "ul_ring: blks_addr :0x%0x, blk_size :%d, fifo_addr :0x%0x, fifo_size :%d, fifo_rdptr :0x%0x, fifo_wrptr: 0x%0x \n",
                                   (uint32_t)fifo_info->blks_addr, (uint32_t)fifo_info->blk_size,
                                   (uint32_t)fifo_info->fifo_addr, (uint32_t)fifo_info->fifo_size,
                                   (uint32_t)fifo_info->fifo_rdptr, (uint32_t)fifo_info->fifo_wrptr);
                        fifo_info = sipx_chan->ul_ack_ring->fifo_info;
                        seq_printf(m, "ul_ack_ring: blks_addr :0x%0x, blk_size :%d, fifo_addr :0x%0x, fifo_size :%d, fifo_rdptr :0x%0x, fifo_wrptr: 0x%0x \n",
                                   (uint32_t)fifo_info->blks_addr, (uint32_t)fifo_info->blk_size,
                                   (uint32_t)fifo_info->fifo_addr, (uint32_t)fifo_info->fifo_size,
                                   (uint32_t)fifo_info->fifo_rdptr, (uint32_t)fifo_info->fifo_wrptr);
...
int  sipx_init_debugfs(void *root )
{
        if (!root)
                return -ENXIO;
        debugfs_create_file("sipx", S_IRUGO, (struct dentry *)root, NULL, &sipx_debug_fops);
        return 0;
}
