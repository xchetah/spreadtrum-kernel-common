...
void get_ddr_info(struct device *dev)
{
#if defined(CONFIG_ARCH_SCX35LT8)
	if (!ddr_base)
		ddr_base = (unsigned long)devm_ioremap_nocache(dev, 0x30000200, 0x200);
#endif
}
...
#if defined(CONFIG_ARCH_SCX35LT8)
	printk(KERN_ERR "REG_AP_CLK_AP_AHB_CFG" ": 0x%08x\n\r", readl(REG_AP_CLK_AP_AHB_CFG));
#endif

	for (i = 0; i < 0x09; i++) {
		printk(KERN_ERR "sdhost" ": 0x%08x | 0x%08x | 0x%08x | 0x%08x\n\r",
		       _sdhost_readl(host->ioaddr, 0 + (i << 4)), _sdhost_readl(host->ioaddr, 4 + (i << 4)),
		       _sdhost_readl(host->ioaddr, 8 + (i << 4)), _sdhost_readl(host->ioaddr, 12 + (i << 4))
		    );
#if defined(CONFIG_ARCH_SCX35LT8)
		printk(KERN_ERR "DDR_CFG" ": 0x%08x\n\r", readl(ddr_base + 0x1fc));
#endif
	}
